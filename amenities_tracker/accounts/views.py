...
def _back_with_query(request, default_name="home"):
    ref = request.META.get("HTTP_REFERER") or reverse(default_name)
...
def register(request):
    ...
    # GET -> open Register pane in modal
    return redirect(_back_with_query(request) + "showRegister=true")
...
def search_amenities(request):
    ...
    if lat and lon:
        for category in categories:
            ...
            if category == "library":
                records = findlibrary(postal_code)
                ...
                for record in records:
                    ...
                    if amenity_lat and amenity_lon:
                        ...
                        if dist <= 1.5:
                            amenities.append({
                                "name": record.get("name"),  # e.g., "Serangoon Public Library"
                                "type": "Library",
                                "address": record.get("address"),
                                "postal_code": record.get("postal_code"),
            ...
            if category == "childcare":
                records = findchildcare(postal_code)
                for record in records:
                    amenity_lat = record.get("latitude")
                    amenity_lon = record.get("longitude")
                    if amenity_lat and amenity_lon and lat and lon:
